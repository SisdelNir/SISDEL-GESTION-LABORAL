 ...
 */
router.post('/', verificarToken, verificarRoot, (req, res) => {
    try {
        const {
            nombre,
        ...
        // Generar código de acceso para el administrador
        ...
        db.prepare(`
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(id_empresa, nombre, identificacion_empresa || '', nombre_administrador, pais || 'MX', moneda || 'MXN', zona_horaria || 'America/Mexico_City', telefono || '', correo || '', direccion || '', codigo_admin);

        // Crear usuario administrador
        db.prepare(`
            INSERT INTO usuarios (id_usuario, id_empresa, identificacion, nombre, telefono, correo, rol, codigo_acceso)
            VALUES (?, ?, ?, ?, ?, ?, 'ADMIN', ?)
        `).run(id_usuario, id_empresa, admin_identificacion || identificacion_empresa || '', nombre_administrador, admin_telefono || telefono || '', admin_correo || correo || '', codigo_admin);

        // Crear configuración por defecto
        db.prepare(`
            INSERT INTO configuraciones_empresa (id_config, id_empresa)
            VALUES (?, ?)
        `).run(id_config, id_empresa);

        // Crear tipos de tarea por defecto
        const tiposDefault = [
        ...
        const insertTipo = db.prepare(`
            INSERT INTO tipos_tarea (id_tipo, id_empresa, nombre, descripcion, peso_complejidad)
            VALUES (?, ?, ?, ?, ?)
        `);

        for (const tipo of tiposDefault) {
            insertTipo.run(uuidv4(), id_empresa, tipo.nombre, tipo.descripcion, tipo.peso);
        }

        // Asignar todos los permisos al ADMIN
        const permisos = db.prepare('SELECT id_permiso FROM permisos').all();
        const insertRolPermiso = db.prepare(`
            INSERT OR IGNORE INTO rol_permisos (id_rol, id_permiso, id_empresa) VALUES (?, ?, ?)
        `);
        for (const p of permisos) {
            insertRolPermiso.run('ADMIN', p.id_permiso, id_empresa);
        }
...
/**
 * GET /api/empresas
 * Listar todas las empresas (solo ROOT)
 */
router.get('/', verificarToken, verificarRoot, (req, res) => {
    try {
...
router.get('/:id', verificarToken, (req, res) => {
    try {
        ...
        // Si no es ROOT, verificar que sea de su empresa
...
router.put('/:id', verificarToken, (req, res) => {
    try {
        ...
        `).run(nombre, identificacion_empresa, pais, moneda, zona_horaria, telefono, correo, direccion, idEmpresa);

        registrarAuditoria(idEmpresa, req.usuario.id_usuario, 'EDITAR_EMPRESA', `Empresa actualizada`);

        const empresaActualizada = db.prepare('SELECT * FROM empresas WHERE id_empresa = ?').get(idEmpresa);
 ...
 */
router.put('/:id/configuracion', verificarToken, (req, res) => {
    try {
        const idEmpresa = req.params.id;
        ...
        const config = db.prepare('SELECT * FROM configuraciones_empresa WHERE id_empresa = ?').get(idEmpresa);
 ...
 */
router.delete('/:id', verificarToken, verificarRoot, (req, res) => {
    try {
        const idEmpresa = req.params.id;

        db.prepare(`UPDATE empresas SET eliminado = 1, fecha_eliminacion = datetime('now','localtime') WHERE id_empresa = ?`)
            .run(idEmpresa);

        // Desactivar todos los usuarios de la empresa
        db.prepare(`UPDATE usuarios SET estado = 0 WHERE id_empresa = ?`).run(idEmpresa);
